...
const setAvailability = async (req, res) => {
  try {
    ...
    if (!date || !timeWindows || timeWindows.length === 0) {
      return res.status(400).json({ message: "Date and timeWindows are required" });
    }
...
const bookSlot = async (req, res) => {
  try {
    ...
    if (!slotId || !name) {
      return res.status(400).json({ message: 'slotId and name are required' });
    }
    ...
    if (alreadyBooked) {
      return res.status(400).json({ message: 'You already have an appointment with this doctor on this day' });
    }
